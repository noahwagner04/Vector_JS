...
*/
Vector.prototype.rotate = function(angleX, angleY, angleZ) {
...
Vector.prototype.distanceFrom = function(vector) {
	/*
	if the passed in argument is not a Vector, 
	make it equal a vector whose coordiantes are (0, 0, 0)
	*/
	vector = vector instanceof Vector ? vector : new Vector();
	/*
...
Vector.prototype.distanceFromSquared = function(vector) {
	/*
	if the passed in argument is not a Vector,
	make it equal a vector whose coordiantes are (0, 0, 0)
	*/
...
Vector.prototype.add = function(vector) {
	// if the provided vector is not a Vector, use an empty one
	vector = vector instanceof Vector ? vector : new Vector();
	this.x += vector.x;
	this.y += vector.y;
	this.z += vector.z;
...
returns this vector with the newly modified coordinates
*/
Vector.prototype.addConst = function(num) {
...
returns this vector with the newly modified coordinates
*/
Vector.prototype.subractConst = function(num) {
	// if the provided number is not a number, default to 0
	num = typeof num === "number" ? num : 0;
	this.x -= num;
	this.y -= num;
